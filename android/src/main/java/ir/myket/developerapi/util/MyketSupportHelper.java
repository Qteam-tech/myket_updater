...
public class MyketSupportHelper {
	...
	private Update getAppUpdateState() throws MyketException {
		checkSetupDone("getAppUpdateState");
		try {
			logDebug("Calling getAppUpdateState for package: " + mPackageName);
			Bundle bundle = mService.getAppUpdateState(DEVELOPER_API_VERSION, mPackageName);

			if (bundle == null) {
				logError("Bundle is null!");
				throw new MyketException(MYKET_BAD_RESPONSE, "Bundle is null");
			}

			logDebug("Bundle keys: " + bundle.keySet());
			logDebug("RESPONSE_CODE: " + bundle.getInt(RESPONSE_CODE));
			logDebug("RESPONSE_APP_UPDATE_AVAILABLE: " + bundle.getBoolean(RESPONSE_APP_UPDATE_AVAILABLE));
			logDebug("RESPONSE_APP_UPDATE_DESCRIPTION: " + bundle.getString(RESPONSE_APP_UPDATE_DESCRIPTION));
			logDebug("RESPONSE_APP_VERSION_CODE: " + bundle.getInt(RESPONSE_APP_VERSION_CODE));

			if (!bundle.containsKey(RESPONSE_CODE) || bundle.getInt(RESPONSE_CODE) !=
					RESPONSE_RESULT_OK) {
				throw new MyketException(MYKET_BAD_RESPONSE, "Response code is not Ok: " + bundle.getInt(RESPONSE_CODE));
			}
			...
			logError("RemoteException: " + e.getMessage());
